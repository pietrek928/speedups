import threading
from collections import namedtuple, defaultdict
from itertools import chain, product
from typing import Iterable, Tuple, Mapping, NamedTuple, Dict

from utils import muli, addi

ctx = threading.local()

Dimension = namedtuple('Dimension', ('n', 'size'))


class VType:
    name: str = '?'
    dims: Tuple[int] = ()

    def __str__(self):
        return self.name
    __repr__ = __str__


OpDescr = NamedTuple('op_descr', (('name', str), ('op_id', int), ('ordered', int), ('out_t', VType)))


class float__(VType):
    name = 'float'


class v4f_(VType):
    name = 'v4f'
    dims = (4, )


class v4x2f_(VType):
    name = 'v4x2f'
    dims = (4, 2)


float_ = float__()
v4f = v4f_()
v4x2f = v4x2f_()

# class NodeType:
#     def __init__(self, name):
#         self.name = name
#
#     def __call__(self, *args, **kwargs):
#         ctx.p.


class CumDims(defaultdict):
    def __init__(self, *a, **k):
        super().__init__(lambda: 1, *a, **k)

    def __mul__(self, d: 'CumDims'):
        m = type(self)()
        for k in set(chain(self.keys(), d.keys())):
            m[k] = self[k] * d[k]
        return m


class ArrayDescr:
    def __init__(self, t: VType, dims: Iterable[Dimension]):
        self._t = t

        vdims = []
        dims = list(dims)
        for i, sz in enumerate(t.dims):
            dim = dims[0]
            vdims.append(Dimension(n=dim.n, size=sz))

            if dim.size == sz:
                del dims[0]
            if not (i == len(t.dims)-1 and dim.size % sz == 0):
                raise ValueError('Dimension {} size invalid'.format(dim.n))
            dims[0] = Dimension(n=dim.n, size=dim.size // sz)

        self._dims: Tuple[Dimension] = tuple(dims)
        self._vdims: Tuple[Dimension] = tuple(vdims)

    def _vec_dims(self, dims: Iterable[Dimension]) -> Tuple[Dimension]:
        dims = tuple(dims)
        assert dims[:len(self._vdims)] == self._vdims
        return dims[len(self._vdims):]

    def _vec_ddims(self, ddims: Mapping[str, int]) -> Dict[str, int]:
        ddims = dict(ddims)
        for vdim in self._vdims:
            if vdim.n in ddims:
                if ddims[vdim.n] % vdim.size:
                    raise ValueError('Dimension {} size not multiple of vector size'.format(vdim.n))
                ddims[vdim.n] //= vdim.size
            else:
                raise ValueError('Lacking dimension {}'.format(vdim.n))
        return ddims

    @staticmethod
    def _aggr_dims(dims: Iterable[Dimension]) -> Dict[str, int]:
        ddims = defaultdict(lambda: 1)
        for dim in dims:
            ddims[dim.n] *= dim.size
        return ddims

    @property
    def dims(self) -> Tuple[Dimension, Dimension]:
        return self._vdims + self._dims

    @property
    def ddims(self) -> Dict[str, int]:
        return self._aggr_dims(self.dims)

    @staticmethod
    def _ddims_poss(ddims: Mapping[str, int]) -> Iterable[Mapping[str, int]]:
        dnames, dsizes = zip(*ddims.items())
        for pos in product(*(range(i) for i in dsizes)):
            yield zip(dnames, pos)

    def _get_idx_vec(self, pos: Mapping[str, int]):
        shift = 1
        pos = defaultdict(lambda: 0, pos)
        idx = 0

        for dim in reversed(self._dims):
            if pos[dim.n] <= 0:
                continue
            v = pos[dim.n] % dim.size
            pos[dim.n] //= dim.size

            idx += v * shift
            shift *= dim.size
        return idx

    def indexes(self, ddims: Mapping[str, int], start=0):
        return self.vec_indexes(self._vec_ddims(ddims), start)

    def vec_indexes(self, vec_ddims: Mapping[str, int], start=0):
        ddims = defaultdict(
            lambda: 1,
            vec_ddims
        )

        if isinstance(start, int):
            start_idx = start
        else:
            start_idx = 0

        items = MemArray(
            dims=(),
            vdims=self._vdims,
            items=(start_idx, )
        )
        shift = 1
        for dim in self._dims:
            if ddims[dim.n] > 1:
                cur_size = min(ddims[dim.n], dim.size)
                if min(ddims[dim.n], dim.size) % cur_size:
                    raise ValueError('Dimension {} is not multiple, and is not taken as last')
                ddims[dim.n] //= cur_size
                items = items.extend(shift, Dimension(n=dim.n, size=cur_size))
            shift = muli(shift, dim.size)

        if start and not start_idx:
            items = items.map(lambda i: addi(start, i))

        return items

    def get_idx(self, vec_pos: Mapping[str, int]):
        vec_pos = self._vec_ddims(vec_pos)
        return self._get_idx_vec(vec_pos)

    def load(self, p, dims: Mapping[str, int], start=0):
        return self.vec_indexes(dims, start).map(
            lambda i: p.load(v4f, i)
        )

    def store(self, a: 'MemArray', start=0):
        idxs = self.indexes(a.ddims, start)
        a = a.reshape(idxs.dims)


class MemArray(ArrayDescr):
    def __init__(self, dims: Iterable[Dimension], vdims: Iterable[Dimension], items: Iterable = None):
        self._dims = tuple(dims)
        self._vdims = tuple(vdims)
        if items is not None:
            self._items = tuple(items)
        else:
            self._items = (None, ) * self.size
        assert self.size == len(self._items)

    @property
    def size(self) -> int:
        n = 1
        for dim in self._dims:
            n *= dim.size
        return n

    def dim_size(self, dim_n: str) -> int:
        n = 1
        for dim in self._dims:
            if dim.n == dim_n:
                n *= dim.size
        return n

    def map(self, func) -> 'MemArray':
        return MemArray(
            dims=self._dims,
            vdims=self._vdims,
            items=(
                func(item) for item in self._items
            )
        )

    def extend(self, shift, dim: Dimension) -> 'MemArray':
        shifted = [self._items]
        cur_shift = shift
        for i in range(1, dim.n):
            shifted.append(
                addi(v, shift) for v in self._items
            )
            cur_shift = addi(cur_shift, shift)
        return MemArray(
            dims=chain(self._dims, (dim, )),
            vdims=self._vdims,
            items=chain(*shifted)
        )

    def reshape(self, dims: Iterable[Dimension]) -> 'MemArray':
        dims = self._vec_dims(dims)
        ddims = self._aggr_dims(dims)
        assert ddims == self._aggr_dims(self._dims)

        r = MemArray(
            dims=dims,
            vdims=self._vdims
        )

        for p in self._ddims_poss(ddims):
            r.set_vec(p, self.get_vec(p))

        return r

    def get_vec(self, pos: Mapping[str, int]):
        return self._items[self._get_idx_vec(pos)]

    def set_vec(self, pos: Mapping[str, int], val):
        self._items[self._get_idx_vec(pos)] = val

    # def __getitem__(self, *items):
    #     return self.get(items)
